...
## Визначення розділів коду для рефакторингу

### Дублювання коду в обробниках маршрутів
- У маршрутах /fetch-select-data/faculties, /fetch-select-data/benefits, /fetch-select-data/specialties/:facultyId тощо є повторювана логіка: запит до бази даних, повернення результату або помилки.
- **Метод рефакторингу**: Метод вилучення — створити узагальнену функцію для обробки таких запитів.

### Довгий метод у /submit_application
- Обробник POST-запиту містить багато логіки: витягування даних із запиту, перевірка їхньої валідності, створення запису. Це ускладнює читабельність і підтримку.
- **Метод рефакторингу**: Метод вилучення — розбити на менші функції (наприклад, для валідації та створення запису).
...
- **Метод рефакторингу**: Заміна магічних чисел на константи — визначити константи для порту та шляхів маршрутів.

### Великий клас Application
- Модель Application має багато полів (15+), що може свідчити про надмірну відповідальність
- **Метод рефакторингу**: Витягнути клас — наприклад, виділити адресу в окрему модель Address.

### Відсутність належної обробки помилок
- У деяких асинхронних операціях помилки обробляються лише через catch, без деталізації.
- **Метод рефакторингу**: Додати детальнішу обробку помилок.